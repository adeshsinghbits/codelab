...
function EventDetailsPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      await dispatch(getSingleEventThunk(event._id)); // Re-fetch
    } catch (err) {
      console.error(err);
    } finally {
      setJoining(false);
  ...
  if (loading || !event) {
    return <div className="text-center p-10 text-gray-500">Loading Event...</div>;
  }
        ...
        <div className="md:w-150 space-y-6">
          ...
          <div className="bg-white shadow-md rounded-lg p-4 mt-6">
            ...
            {attendees.length > 0 ? (
              <ul className="space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
                {attendees.map((attendee) => (
                  ...
                  >
                    ...
                    />
                    <div>
                      <p className="font-semibold text-gray-700">{attendee.name}</p>
              ...
              <p className="text-gray-500 italic">No attendees have joined yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="border rounded-lg p-6 bg-white shadow-sm text-center">
            {isJoined ? (
              <button
                onClick={handleLeave}
                className="bg-red-500 hover:bg-red-600 text-white font-bold px-4 py-2 cursor-pointer rounded shadow-md"
              >
              ...
              <button
                onClick={handleJoin}
                className="bg-green-500 hover:bg-green-600 text-white font-bold px-4 py-2 cursor-pointer rounded shadow-md"
              >
            ...
            <p className="text-sm text-gray-500 mt-2">
              RSVP by {dayjs(event.startDate).subtract(1, "day").format("MMM D, YYYY")}
            </p>
          </div>
